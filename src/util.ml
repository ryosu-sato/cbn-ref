...
let print_list_with_flag
      ?(bopen = Format.pp_open_box -$- 0)
      ?(bclose = Format.pp_close_box -$- ())
      print
      ?(first = false)
      ?(last = false)
      ?limit
...
let print_list
  ...
  =
  let print ~first ~last fm x = print fm x in
  print_list_with_flag ~bopen ~bclose print ~first ~last ?limit punc fm xs
  [@@ocaml.warning "-27"]
...
module List = struct
  ...
  let remove_all ?(eq=(=)) xs x = filter_out (eq x) xs
  ...
  module Make(X : EqType) = struct
    ...
    let remove_all xs x = remove_all ~eq:X.eq xs x
    module Set = struct
  ...
  module L = struct
    ...
    let flatten_map ~f xs = concat_map f xs
